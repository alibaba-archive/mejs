...
function Template(text, options) {
  ...
  this.delimiter = escapeRegExpChars(options.delimiter || _DEFAULT_DELIMITER);
  this.rmWhitespace = options.rmWhitespace; //TODO
...
Template.prototype.compile = function() {
  this.generateSource();
  var append = 'var ctx = this, __output = "";\n  ';
  if (_INCLUDE.test(this.source))
    append += 'var include = function(tplName, data) { return ctx.render(ctx.resolve(__tplName, tplName), ctx.copy(it, data)); }\n  ';
...
Template.prototype.scanLine = function(line) {
  ...
  switch (line) {
    ...
    case d + '>':
    case '-' + d + '>':
      if (this.mode === _MODES.LITERAL) _addOutput();
      this.mode = null;
    ...
    default:
      ...
      switch (this.mode) {
        ...
        case _MODES.ESCAPED:
          ...
          this.source += ';__output += ctx.escape(' +
...
function createRegex(delimiter) {
  var delim = _REGEX_STRING.replace(/%/g, delimiter);
