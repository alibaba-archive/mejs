...
Template.prototype.parseTemplateText = function () {
  ...
  while (res) {
    if (opening) {
      ...
      if (res[0] !== d + '>' && res[0] !== '-' + d + '>')) break
      opening = ''
    } else if (res[0].indexOf('<' + d) === 0 && res[0].indexOf('<' + d + d) !== 0) {
      opening = res[0]
    }
    ...
    default:
      ...
      switch (this.mode) {
        ...
        case _MODES.RAW:
          ...
          this.source += ';__output += [' + line.replace(_TRAILING_SEMCOL, '').trim() + '].join("")'
          break
        case _MODES.COMMENT:
          // Do nothing
          break
